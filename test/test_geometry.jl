...
p2 = Point(2.0)
l1 = Line{2}((1,2), (p1, p2))
@test typeof(l1) <: Line{2, 1}
@test length(l1.nodes) == 2

p3 = Point(3.0)
l2 = Line{3}((1,2,3), (p1,p2,p3))
@test typeof(l2) <: Line{3, 1}
@test length(l2.nodes) == 3

p1 = Point(0.0, 0.0)
p2 = Point(1.0, 0.0)
l1 = Line{2}((1,2), (p1, p2))
@test typeof(l1) <: Line{2, 2}
@test length(l1.nodes) == 2

p3 = Point(0.5, 0.0)
l2 = Line{3}((1,2,3), (p1, p2, p3))
@test typeof(l2) <: Line{3, 2}
@test length(l2.nodes) == 3
...
p5 = 0.5*(p2 + p3)
p6 = 0.5*(p1 + p3)
t2 = Triangle{6}((1,2,3,4,5,6),
				(p1,p2,p3,p4,p5,p6))
@test typeof(t2) <: Triangle{6, 2}
...
p3 = Point(1.0, 1.0)
p4 = Point(0.0, 1.0)
q1 = Quadrilateral{4}((1,2,3,4),
			(p1,p2,p3,p4))
@test typeof(q1) <: Quadrilateral{4, 2}
...
p8 = 0.5*(p4 + p1)
p9 = Point(0.5, 0.5)
q2 = Quadrilateral{9}((1,2,3,4,5,6,7,8,9),
			(p1,p2,p3,p4,p5,p6,p7,p8,p9))
@test typeof(q2) <: Quadrilateral{9, 2}
...
@test haskey(mesh.element_types, Triangle{3})
@test haskey(mesh.element_types, Line{2})
@test haskey(mesh.element_types, Quadrilateral{4})
